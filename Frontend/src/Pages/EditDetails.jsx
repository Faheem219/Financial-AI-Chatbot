...
const EditDetails = () => {
    ...
    const [investmentGoals, setInvestmentGoals] = useState(user ? user.investment_goals : '');
    const [riskTolerance, setRiskTolerance] = useState(user ? user.risk_tolerance : 'medium');
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            // updatedUser now contains the newly updated data from the backend
            // update the AuthContext user
            login(updatedUser, token);

            // Redirect to the dashboard (or anywhere you'd like)
            navigate('/dashboard');
        } catch (err) {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-100">
            <form onSubmit={handleUpdate} className="max-w-md w-full bg-white p-8 shadow-md rounded">
